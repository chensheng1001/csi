...
#     :param data_dir: The path to the folder that contains all data.4 
#     :param data: The DataFrame that contains entries which should be in this dataset.
#     :param class_num: The number of classes.
#     :param gram_type: which type of gram is used for features.
#     :param slicer: The slicer that turn a gram into a :class:`numpy.ndarray` of gram snippets. If not provided,
...
#         user_label = self.user_labels[ind]
#         user_onehot_label = numpy.array([1 if i == user_label else 0 for i in range(self.class_num)]).astype(numpy.int64)
#         pos_label = self.pos_labels[ind]

#         returns = [sample, user_label, user_onehot_label, room_label, pos_label]
...
#             data_dir = pathlib.Path('/workspace/data/gram_data_v6/all_gram'),
#             class_num = 8,
#             gram_type = "log_stft",
#             rooms_list = [[i for i in range(4) if i != 3], [3]],
#             slicing = True,
#             slice_length = 400, slice_stride = 200, additional_labels = True)
#     print(len(set1))
#     print(len(set2))
#     loader1 = get_data_loader(set1, 16, False, 1)
#     batch = next(iter(loader1))
#     for tensor in batch:
#         print(tensor.shape, tensor.dtype)
#     set1, = get_datasets(
#             data_dir = "/workspace/gram_data",
#             class_num = 8,
#             gram_type = "log_stft",
...
#             slice_length = 400, slice_stride = 200, additional_labels = True)
#     print(len(set1))
#     loader1 = get_data_loader(set1, 16, 0)
#     batch = next(iter(loader1))
#     for tensor in batch:
#         print(tensor.shape, tensor.dtype)
...
class MyDataset(MyBaseDataset):
    ...
    def __getitem__(self, ind) -> Tuple[torch.Tensor, ...]:
        ...
        returns = [sample, user_label, user_onehot_label, room_label]
...
def get_data_loader(dataset: tor_data.Dataset, batch_size: int, drop_last: bool = False,
    ...
    return tor_data.DataLoader(
            dataset, batch_size = batch_size,
            shuffle = True, drop_last = drop_last,
            num_workers = worker_num, worker_init_fn = _worker_init_fn, pin_memory = False)